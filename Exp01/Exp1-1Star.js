...
window.onload = () => {
    ...
    // 向缓冲区写入顶点数据 | Write vertex data to the buffer
    ...
    // 将缓冲区中的数据传递给顶点着色器中的a_Position变量
    // Pass the data in the buffer to the a_Position variable in the vertex shader
...
function render(gl) {
    gl.clear(gl.COLOR_BUFFER_BIT);  // 用背景色擦除窗口颜色 | Erase the window color with background color
    // gl.drawArrays(gl.POINTS, 0, vertices.length);
    // gl.drawArrays(gl.LINE_LOOP, 0, vertices.length / 2);
    // 绘制图元类型为三角扇, 从第零个顶点开始, 绘制vertices.length个
    // Draw the primitive type as a triangle fan, starting from the zeroth vertex, and draw vertices.length
